...
```bash
# Build and run all tests (751 assertions expected, 0 failures)
...
CYRIUS_DCE=1 cyrius build --features tpm -D LIBRO_TPM src/main.cyr build/libro_tpm && \
    ./build/libro_tpm   # 763 assertions: 751 default + 12 TPM-gated
...
**Total: 751 assertions default / 763 with `-D LIBRO_TPM`** across
...
| TPM-opt-in build check | 2.5.0 | `-D LIBRO_TPM` build + tests pass (751 → 763 assertions) |
