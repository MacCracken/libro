...
```bash
# Build and run all tests (795 assertions expected, 0 failures)
...
CYRIUS_DCE=1 cyrius build --features tpm -D LIBRO_TPM src/main.cyr build/libro_tpm && \
    ./build/libro_tpm   # 807 assertions: 795 default + 12 TPM-gated
...
**Total: 795 assertions default / 807 with `-D LIBRO_TPM`** across
